...
| Addata | 要る | 要らない | 要らない |
| 部品コード・標準指数 | 入る | 入らない | **入らない**（2026-09-22 から照合しない。品番は表の「部品番号」だけ。古い照合器が左右・前後違いの部品コードを入れていた） |
| 税込/税抜 | 合計欄から**自動判定** | 画面のラジオ（手動） | 画面のラジオ（手動） |
| 原本との照合 | する | する | **できない**（CSV の印字の小計・AI の相違申告・行ごとの点検だけ） |
| 区分が空欄の行 | スキルの規則 | 品名から推し量る | **区分なし（-1）のまま**（推し量れば区分が付く行は取り込み時に知らせる。プレビュー取り込みは取り込むときに推し量った区分を表に入れ、④は表のとおり） |
| サイドバーの費用 | 使わない | チェックを入れたときだけ | ③の「💰 合計・費用」でチェックを入れたときだけ |
