...
def to_halfwidth(s: str) -> str:
    """全角英数カナ → 半角（NEO の名称欄・CarName 用）。"""
    # 文字列ごと NFKC する（1 文字ずつだと半角の 'ﾊ' + 'ﾞ' が結合されず、結合用濁点 U+3099 が残って
    # cp932 に encode できなくなる。ADDATA は cp932 なので 'ﾊｲﾌﾞﾘｯﾄﾞ' のような半角濁点が素で来る）
    txt = unicodedata.normalize('NFKC', s or '')
    # 全角カタカナ → 半角カタカナ（中黒も半角に。下書きの _hw_kana と揃える。NFKC が '･' を '・' に上げるので必要）
