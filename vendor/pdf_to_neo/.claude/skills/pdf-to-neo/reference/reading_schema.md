...
flags: `M` = manual（ADDATA に無い品目）、`R` = reserve（保留）、`N` = 注記行（name を note として扱う）、`$` `#` `*` `@` = コグニ印刷の右端の印（`@` は板金ランク行）（そのまま写す。make_neo が生成結果の WageByManual と照合し、違う行を表示する）。
**印は必ず写す**: `#`（手入力指数）が付いた行は、指数の列が無い書式（コグニ印刷は工賃だけ）でも下書きが 指数 = 工賃 ÷ レバーレート を起こす（0.1 刻みに乗り、その指数から工場の丸め単位で工賃を計算し直すと印字に戻るときだけ。採否は `_draft_notes` に出る）。写さないと生成器は「工賃だけ手入力」（指数なし・印 `*`）の行にするので、コグニ印刷と印・指数が食い違う（2026-09-16）。`*` の行は起こさない。数値のカンマは無視。dict と混在可（dict では `"mark": "*#"`）。
...
| rows[].comment | 任意。**転記メモ**（NEO には書かない。確認箇所シートに載る）。`要確認:` で始めるか `?` を含むと 要確認、それ以外は 参考。見積書に**印字された明細コメント**だけは先頭に `NEO:`（全角可）を付ける → NEO の明細コメント（40 バイト）。dict 行は `neo_comment` でも可（2026-09-13） |
| rows[].neo_name | 任意（dict 行のみ）。**手入力の行**（M・汎用車種）の NEO の名称欄（cp932 で 24 バイト）に入れる短い名前。印字の名称が 24 バイトを超えるときに書く（reading_check が WARN で知らせる）。書かなければ下書きが自動で短くし（句読点を半角・空白を詰める・括弧書きを外す・切る）、確認箇所シートの 要確認 になる。部品コードのある行は ADDATA の標準名称になるので効かない |
| rows[].reserve / area | 任意。`area` は板金の面積 d㎡（名称の `(6dm²)` でも可。小数は四捨五入） |
| 「〃」の行 | 名称が `〃 交換工賃`（`〃 工賃` / `〃 脱着工賃` 等）で金額の無い行は、下書きが直前の部品の行の工賃にまとめる（まとめずに印字どおり写してよい）。`〃（モデリスタ）` のように金額のある行は、直前の名称を補った別の行になる |
| 数量 1 のまま複数個分の金額 | 印字どおり写す。下書きが ADDATA の標準単価（3,000 円以下の小物）の整数倍（2〜99 倍）なら数量を直し、確認箇所シートに「数量」として載せる（判断規則 10-21） |
...
| paint.material / material_rate / total | 材料計・割合（材料計 ÷ 塗装工賃計）・塗装工賃計。材料計が 塗装工賃計 × 割合 の一括四捨五入と一致すれば draft は割合だけ渡す（コグニの費用割合モード）。一致しなければ材料計を手入力 `*` として渡す。**`total` はコグニ印刷の「塗装工賃計」**: 内板骨格塗装（`paint.frame`）を含み、追加項目（`paint.other`。印刷の「追加塗装費用計」＝プライマー塗装など）を含まない（塗装費用計 = 塗装工賃計 + 塗装材料代計 + 追加塗装費用計。判断規則 10-25） |
| paint.frame | 内板骨格塗装。位置ごとに `{"engine_room": {"option": 2, "index": 1.5, "wage": 13130}}`（エンジンルーム 1 = ラジエータサポート両側新品または修正 / 2 = ＋Fフェンダエプロン片側 / 3 = ＋両側、`front_pillar`・`center_pillar` 1 = 片側新品 2 = 両側新品、`rear_floor` 1 = 1 台小修正 2 = 大修正）。印刷では「ラジエータサポート 両側新品 1.50」「フロントフェンダエプロン 片側新品または修正」のように 2 行で出る |
| paint.other | 追加項目（工程名。材料代の対象外）。`[{"name": "プライマー塗装", "index": 1.7, "wage": 14880}]` |
| expenses[].in | その費用が入っている合計欄の名前（部品計 / 作業計 / 諸費用計 / 非課税）。script が kind に写す。**部品と工賃の両方に金額がある費用**（「ﾓﾃﾞﾘｽﾀｻｲﾄﾞｽｶｰﾄ 36,000 / 5,000」）は同じ名前で 2 行（`in` 部品計と作業計）に書く。生成器が 1 行の費用（部品・工賃の両欄）にまとめる |
...
| キー | 例 | 何に効くか |
|---|---|---|
| `wage_round` | `100` | 工賃の丸め単位（1/10/100 円）。draft が印字工賃から自動推定するので普通は書かない（指数×レートをそのまま 1 円単位で刷る書式 — トヨタ系ディーラーの 0.90×7,820 = 7,038 — は 1 と判定する。2026-09-14） |
