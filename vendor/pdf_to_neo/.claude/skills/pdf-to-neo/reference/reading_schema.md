...
| paint.material_round | 材料代の端数処理（工場ごと）。`{"unit": 10, "mode": "切り上げ"}` か `"1円四捨五入"`。**ふつうは書かない** — 下書きが印字の材料代から割り出す（既定は 10 円四捨五入。`reference/painting.md` §7） |
| paint.input_type | 塗装の入力方式 `"指数"` / `"実額"` / `"参考"`（コグニ: その他 → 塗装 → 入力方式）。一式しか印字が無い見積は書かなくても下書きが **実額**にする（`painting.md` §2） |
| paint.frame | 内板骨格塗装（コグニの塗装画面 内板骨格タブ）。**印字の塗装明細にある行はそのまま `lines` に写せばよい**（下書きが「ラジエータサポート 両側新品」「フロントフェンダエプロン 片側新品または修正」「フロントピラー 片側新品」「リヤフロア １台小修正」を振り分ける。`painting.md` §6）。直接書くときは位置ごとに `{"engine_room": {"option": 2, "index": 1.5, "wage": 13130}}`（エンジンルーム 1 = ラジエータサポート両側新品または修正 / 2 = ＋Fフェンダエプロン片側 / 3 = ＋両側、`front_pillar`・`center_pillar` 1 = 片側新品 2 = 両側新品、`rear_floor` 1 = 1 台小修正 2 = 大修正）。印刷では「ラジエータサポート 両側新品 1.50」「フロントフェンダエプロン 片側新品または修正」のように 2 行で出る |
...
| `frame` | `{"page": 5, "basic": true, "basic_wage": 28000, "items": [{"code": "1388", "rank": "B", "wage": 12000}]}` | 内板骨格修正（estimate_schema の frame と同じ形）。`page` は【内板骨格修正】の区画が印字されたページ（検算がそのページの小計の別解「明細 + 内板骨格」にだけ使う。無ければ最初に合った 1 ページだけ）。この区画の行は明細（rows）に写さない |
