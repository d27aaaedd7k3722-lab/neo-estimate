...
2-2. flags の `M`（手入力）は、4 桁の部品コードが印字されている見積で**コード欄が空の行**と、素材欄に `*` が印字された行だけに付ける。
   部品コードが 1 つも印字されていない見積（協定見積書・他システムの見積）では付けない（ADDATA に有るかはプログラムが照合して決める）
3. 金額は数量分。単価しか印字が無ければ price に 単価×数量、comment に `unit=単価`。数量 1 のまま複数個分の金額は印字どおり。
   qty は数量の欄の印字だけ（名前の「(2個)」は部品の入り数で名前の一部。数量の欄が空なら qty も空）
3-2. method には**区分の語だけ**（区分の欄に印字された語: 取替・脱着・修理・修正・板金・脱着修理・点検調整・分解調整 など。
   再封印・施工 のようにコグニに無い語も印字どおり method に。名前に続けて印字されていれば名前のまま）。
...
3-4. **金額は税込・税抜を気にせず印字どおりに写す**。各行の金額まで税込で刷る書式（ディーラー・二輪）でも 1.1 で割らない。
   税込かどうかはプログラムが合計欄と明細の積み上げで見分けて、あとから税抜に直す（勝手に割るとページ小計と合わなくなる）
4. 左右・Fr/Rr・上下は印字どおり
...
6. このページに塗装の区画（外板パネル・バンパ・加算基礎数値・ブース・付加塗装・材料代 など）や費用の区画（ショートパーツ・廃棄費用・
   ...
   【内板骨格修正】（骨格修正）の区画の行（基本修正作業・部位ごとの修正）も rows に入れない（header の frame に書く。rows_printed にも数えない）。
...
6-2. 費用の区画が**印字されていない**見積（明細の表の続きに雑多な行が並ぶだけ）では、**作業区分が付いているかで分ける**。
   「リヤナンバー再封印」「ドライブレコーダーリヤカメラ脱着」「ソナー点検・調整」のように
   **名前に作業区分（取替・脱着・修理・板金・交換・点検・調整・再封印 など）が付いた行は、費用らしい名前でも明細（rows）** に写す
   （コグニでは修理方法を入れた手入力の明細行になる。実機の協定見積書で確認）。
   expenses に写すのは **作業区分が付かず金額だけの行**（写真代・ショートパーツ・部品廃棄料・車両回送費・保管料 など）。
   区分は**印字の語をそのまま**（「再封印」を「脱着」に言い換えない。部品番号も指数も無い手入力の作業行なら、コグニに無い語でもそのまま入る）。
   金額はどちらに写しても合計は合うので検算では気づけない。**NEO のどの画面に出るかが変わる**ので、印字のとおりに分ける
