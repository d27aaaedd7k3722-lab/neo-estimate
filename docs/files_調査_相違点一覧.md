...
**片方だけ直っています。** ただし影響は限定的です（2026-09-14 バグハントで再確認）:
部品コード（`auto_matching.py:1562` `_code_ok = level in ('L1','L2') and pdf_price > 0`）も
品番の補い（`app.py:1142` `match_level in ('L1','L2') and parts_amount > 0`）も **見積に金額のある行だけ**に絞られているので、
価格の無い行が L2 になっても部品コード・品番は入りません。残る差は「※ 印が出ない（名称だけの照合だと分からない）」だけです。
